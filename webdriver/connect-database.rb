...
def garment_type
  ...
  matched_words = []
  ["jacket", "dress", "top", "shorts", "pants", "petite", "blouse", "shirt", "tunic", "coat", "cape", "jumpsuit", "skirt", "vest", "robe", "gown", "corset", "maternity", "jeans"].each do |fancy_word|
...
urls = ['http://voguepatterns.mccall.com/v1362-products-47539.php?page_id=316']
urls.each do |url|  
...
#rs = con.query("select id from collection where name = 'Anne Klein'")
...
#get pattern collection name
#if pattern collection name exists in pattern collection table
#    get pattern collection name id
#else 
#    add pattern collection name to pattern collection table
#    get pattern collection name id
